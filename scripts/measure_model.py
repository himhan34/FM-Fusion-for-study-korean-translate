...
def create_measurements(detections):
    ...
    for k,det in enumerate(detections):
        ...
        measurments.append({'k':k,'labels':det.labels})
...
def process_scene(args):
    ...
    for index,pred_frame in enumerate(sorted(predict_frames)):   
        ...
        frame_association['prompts'] = create_prompts(tags)
        ...
        print('{}/{} instances are matched'.format(np.sum(assignment_det_gt),viewed_instances))
...
if __name__=='__main__':
    ...
    split='train'
    PREDICT = 'prediction_prompts'
    result_folder = os.path.join(dataroot,'measurements','prompts')
    ...
    # for scan in scans:
    #     process_scene((os.path.join(dataroot,split,scan),result_folder,PREDICT))
    #     break
    # exit(0)
    
    import multiprocessing as mp
    p = mp.Pool(processes=32)
    p.map(process_scene, [(os.path.join(dataroot,split,scan), result_folder, PREDICT) for scan in valid_scans])
    p.close()
    p.join()
