...
int main() {
  ...
  std::string dataset_dir =
      "/media/qzj/Document/datasets/fmfusion/pose_graph/" + frame_pair;
  ...
  params.dist_trans_threshold = 0.8;
  ...
  std::cout << "GT T_ref_src: " << T_ref_src.matrix() << std::endl;
  Eigen::Matrix4d T_ref_src_est =
      ref_poses[0] *
      output_vals.at<gtsam::Pose3>(gtsam::Symbol('b', 0)).matrix().inverse();
  std::cout << "Est T_ref_src: " << T_ref_src_est << std::endl;
