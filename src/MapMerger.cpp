...
int main(int argc, char *argv[]) 
{
    ...
    auto global_semantic_map = std::make_shared<fmfusion::SemanticMapping>(fmfusion::SemanticMapping(sg_config->mapping_cfg, sg_config->instance_cfg));
    ...
    for (auto it=pose_map.begin();it!=pose_map.end();it++){
        ...
        auto local_sg = std::make_shared<fmfusion::SemanticMapping>(fmfusion::SemanticMapping(sg_config->mapping_cfg, sg_config->instance_cfg));
        ...
        global_semantic_map->merge_other_instances(local_instances);

        if(count>0) global_semantic_map->merge_overlap_instances();

        count ++;
        // break;
    }
    global_semantic_map->extract_bounding_boxes();

    //
    if (visualization){
        auto geometries = global_semantic_map->get_geometries(true,true);
        open3d::visualization::DrawGeometries(geometries, scene_name, 1920, 1080);
    }
    
    // Export 
    global_semantic_map->Save(map_folder+"/"+scene_name);
